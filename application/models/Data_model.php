...
class Data_model extends CI_Model
{
    ...
    public function hitungJumlahTotal()
    {
        $this->db->select_sum('JUMLAH_BAYAR');
        $query = $this->db->get_where('tbl_pembayaran');
        if($query->num_rows()> 0)
        {
    ...
    public function transaksi_get()
    {
        ...
        `tbl_pembayaran`.`nisn` = `tbl_siswa`.`nisn` AND `tbl_siswa`.`id_kelas` = `tbl_kelas`.`id_kelas` AND `tbl_siswa`.`id_spp` = `tbl_spp`.`id_spp` AND `tbl_kelas`.`id_jurusan` = `tbl_jurusan`.`id_jurusan` AND `tbl_pembayaran`.`KET` = 'LUNAS' 
